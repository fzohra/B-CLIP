...
def eval_sharegpt4v_retrieval(val_loader, model, tokenizer, device):
    ...
    iterator = tqdm(val_loader, desc="ShareGPT4V retrieval")
...
def eval_sharegpt4v_retrieval_tci(val_loader, model, tokenizer, device):
    ...
    iterator = tqdm(val_loader, desc="ShareGPT4V TCI/CLS")
...
def validate_sharegpt4v(model, output_dir, epoch, step, log_wandb, tokenizer, device='cuda'):
    model.eval()
    m = unwrap_model(model)
    ...
    # Create validation dataset using the existing class
    val_dataset = datasets.sharegpt4v_val_dataset(
    ...
    batch_size = 1000 // world_size  # Small batch size to avoid OOM
        
    # Show how many samples each rank should process
    if is_dist():
        samples_per_gpu = 1000 // world_size
        remaining_samples = 1000 % world_size
    ...
    if is_main_process():
        ...
        if log_wandb:
            wandb_payload = {}
            if 'retrieval_acc_tci_t2i' in results:
                wandb_payload["sharegpt4v_retrieval_tci_t2i"] = results['retrieval_acc_tci_t2i']
            if 'retrieval_acc_tci_i2t' in results:
                wandb_payload["sharegpt4v_retrieval_tci_i2t"] = results['retrieval_acc_tci_i2t']
            if 'retrieval_acc_cls_t2i' in results:
                wandb_payload["sharegpt4v_retrieval_cls_t2i"] = results['retrieval_acc_cls_t2i']
            if 'retrieval_acc_cls_i2t' in results:
                wandb_payload["sharegpt4v_retrieval_cls_i2t"] = results['retrieval_acc_cls_i2t']
            if wandb_payload:  # Only log if there are metrics
